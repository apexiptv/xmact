...
def process(device_id: str):
    ...
    results['crm'] = get_crm_account_plan_information(device_id, auth_token, seq)
    db_update_for_google()
    block_list_device(auth_token)
    results['create'] = create_account(device_id, auth_token, seq)
    results['refresh'] = update_device_sat_refresh_with_priority_cc(device_id, auth_token)
    db_update_for_google()
